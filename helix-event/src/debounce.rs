...
/// debounce their events, coordinate multiple differen hooks and potentially
/// track some state. `AsyncHooks` facilitate these usecase by running as
/// a background tokio task that waits for events (usually an enum) to be
/// send trough a channel.
